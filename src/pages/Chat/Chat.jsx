...
export default function Chat() {
  ...
  const checkAndSetLanguage = async () => {
    ...
    if (userDocSnap.exists()) {
      ...
      if (userData.language === null) {
        // Language is not set, open the language selection dialog
  ...
  const handleLanguageSelection = async (selectedLanguage) => {
    // Validate and update the user's language in Firestore
    if (["English", "Hindi", "Marathi"].includes(selectedLanguage)) {
      const userDocRef = doc(db, "users", user.uid);
      ...
      alert(
        "Invalid language selection. Please choose from English, Hindi, or Marathi."
      );
    }
  };
  ...
  return (
    <Box sx={{ display: "flex", height: "100%", position: "relative" }}>
      ...
      <Dialog open={isLanguageDialogOpen} onClose={handleLanguageDialogClose}>
        <DialogTitle>Select Your Language</DialogTitle>
        <DialogContent>
          <Button onClick={() => handleLanguageSelection("English")}>
            English
          </Button>
          <Button onClick={() => handleLanguageSelection("Hindi")}>
            Hindi
          </Button>
          <Button onClick={() => handleLanguageSelection("Marathi")}>
            Marathi
          </Button>
        </DialogContent>
        <DialogActions>
